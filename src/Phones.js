...
const Phones = () => {
  return (
    <div className="phone-page">
      <h1>Phones</h1>
      <div className="phone-list" style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
        {phoneData.map((phone) => (
          <div className="phone-item" key={phone.id} style={{ width: '200px' }}>
            <Link to={`/phone/${phone.id}`}>
              ...
              />
              <h2 style={{ fontSize: '1rem', textAlign: 'center' }}>{phone.name}</h2>
            </Link>
          </div>
        ))}
      </div>
